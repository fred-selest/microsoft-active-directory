...
from ldap3 import Server, Connection, ALL, SUBTREE, MODIFY_REPLACE, MODIFY_ADD, MODIFY_DELETE, Tls, NTLM, SIMPLE
import ssl
...
def get_user_role_from_groups(conn, username, debug=False):
    ...
    try:
        ...
        # Extraire le nom d'utilisateur sans domaine
        ...
        # Extraire les groupes (CN depuis les DN)
        user_groups = []
...
def get_ad_connection(server=None, username=None, password=None, use_ssl=False, port=None):
    ...
    try:
        ...
        if ntlm_err and 'strongerAuthRequired' in ntlm_err and not use_ssl:
            # Réessayer avec SSL sur le port 636
            ssl_port = 636
            ad_server_ssl = Server(
                server,
                port=ssl_port,
                use_ssl=True,
                tls=tls_config,
            ...
            if conn:
                # Mettre à jour la session pour utiliser SSL à l'avenir
            ...
            # Si SSL échoue aussi, essayer STARTTLS sur le port original
            try:
            ...
            return None, f"Le serveur exige une authentification sécurisée (SSL/TLS). Erreur: {ssl_err or ssl_err2}"
...
def connect():
    ...
    if request.method == 'POST':
        ...
        if conn:
            ...
            # Afficher les groupes AD détectés
            if debug_info.get('groups'):
                flash(f'Groupes AD: {", ".join(debug_info["groups"][:5])}', 'info')
            if debug_info.get('error'):
                flash(f'Erreur groupes: {debug_info["error"]}', 'warning')
            return redirect(url_for('dashboard'))
        else:
