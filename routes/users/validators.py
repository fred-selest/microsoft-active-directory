...
class UserCreateRequest:
    ...
    def validate(self) -> List[str]:
        ...
        # Validation OU - optionnelle (le backend utilise base_dn par défaut)
        if self.ou and self.ou.strip():
            if not self.ou.startswith('DC=') and not self.ou.startswith('OU=') and not self.ou.startswith('CN='):
